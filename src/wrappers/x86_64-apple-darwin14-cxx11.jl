# Autogenerated wrapper script for Singular_jll for x86_64-apple-darwin14-cxx11
...
const libfactory_splitpath = ["lib", "libfactory.dylib"]
...
const libfactory = "@rpath/libfactory-4.1.3.dylib"


# Relative path to `libomalloc`
const libomalloc_splitpath = ["lib", "libomalloc.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libomalloc = "@rpath/libomalloc-0.9.6.dylib"
...
const libpolys_splitpath = ["lib", "libpolys.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libpolys = "@rpath/libpolys-4.1.3.dylib"


# Relative path to `libsingular`
const libsingular_splitpath = ["lib", "libSingular.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libsingular = "@rpath/libSingular-4.1.3.dylib"


# Relative path to `libsingular_resources`
const libsingular_resources_splitpath = ["lib", "libsingular_resources.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libsingular_resources = "@rpath/libsingular_resources-4.1.3.dylib"
